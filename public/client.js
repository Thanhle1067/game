...
const barsEl = document.getElementById('bars'), counterEl = document.getElementById('counter'), bonusEl = document.getElementById('bonus'), stageTag = document.getElementById('stageTag');
...
let particles=[]; function spawn(x,y,c=60){ for(let i=0;i<c;i++){ const a=Math.random()*Math.PI*2, s=2+Math.random()*4; particles.push({x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:40+Math.random()*20,age:0}); } }
(function loop(){ ctx.clearRect(0,0,fx.width,fx.height); particles=particles.filter(p=>p.age<p.life); for(const p of particles){ p.age++; p.vy+=0.04; p.x+=p.vx; p.y+=p.vy; const t=1-p.age/p.life; ctx.globalAlpha=t; ctx.fillStyle='white'; ctx.fillRect(p.x,p.y,3,3);} requestAnimationFrame(loop) })();
function burst(){ const cx=fx.width/2, cy=fx.height*0.4; spawn(cx,cy,80); setTimeout(()=>spawn(cx-150,cy+40,60),120); setTimeout(()=>spawn(cx+160,cy+10,60),180); }
...
function applyStage(n){ if(stage===n) return; stage=n; document.body.classList.remove('stage1','stage2','stage3'); document.body.classList.add('stage'+stage); stageTag.textContent='Tầng '+stage; }
...
function shake(){ barsEl.classList.add('shake'); setTimeout(()=>barsEl.classList.remove('shake'),650); }
...
socket.on('hello', ()=>console.log('connected')); applyStage(1); render();