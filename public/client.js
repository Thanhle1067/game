const socket=io();
let seconds=0, active=false, barsShown=false, zeroBurstDone=false, stage=1;
const baseBars=document.getElementById('bars'),root=document.getElementById('root');
const fireEl=document.getElementById('fire');const counterWrap=document.getElementById('counterWrap');
...
function swapBars(img){const incoming=document.createElement('div');incoming.className='bars next';incoming.style.backgroundImage=`url(${img})`;root.appendChild(incoming);
  incoming.addEventListener('animationend',()=>{baseBars.classList.remove('show');baseBars.style.opacity='0';baseBars.style.backgroundImage=`url(${img})`;void baseBars.offsetWidth;baseBars.classList.add('show');baseBars.style.opacity='1';root.removeChild(incoming);},{once:true});}
function showBars(){baseBars.style.display='block';if(!barsShown){barsShown=true;baseBars.classList.add('show');}}
function hideBars(){baseBars.classList.remove('show');baseBars.style.display='none';barsShown=false;}
function showUI(){counterWrap.style.display='block';fx.style.display='block';}
function hideUI(){counterWrap.style.display='none';fx.style.display='none';fireEl.style.display='none';hideBars();}
...
function updateDecor(){ if(stage>=2){ fireEl.style.display='block'; fireEl.style.height=(stage===3?'230px':'180px'); } else { fireEl.style.display='none'; } }
function updateStage(){if(seconds>=T3)applyStage(3);else if(seconds>=T2)applyStage(2);else applyStage(1);updateDecor();}
setInterval(()=>{if(!active) return;if(seconds>0){if(!barsShown)showBars();seconds-=1;if(seconds<=0){seconds=0;active=false;hideUI();if(!zeroBurstDone){zeroBurstDone=true;burst();}}updateStage();render();}},1000);
socket.on('gift',d=>{const isRescue=d?.type==='rescue';const hasSec=d?.sec!==undefined&&d.sec!==null&&d.sec!=='';let delta=hasSec?parseInt(d.sec,10):10*parseInt(d?.amount||1,10);if(isRescue&&delta>0)delta=-delta;
 // nếu chưa active: chỉ bắt đầu khi delta>0
 if(!active && delta>0){ active=true; showUI(); showBars(); zeroBurstDone=false; }
 // nếu chưa active mà là rescue hoặc delta<=0 -> bỏ qua
 if(!active) { render(); return; }
 if(isRescue){bomb();shake();try{clack.currentTime=0;clack.play();}catch(e){}}
 const wasPos=seconds>0; seconds=Math.max(0,seconds+delta);
 if(delta>0){flash('+'+delta+'s');try{ting.currentTime=0;ting.play();}catch(e){};zeroBurstDone=false;} else if(delta<0){flash(delta+'s');}
 if(seconds>0)showBars();else{hideBars();active=false;hideUI();if(wasPos&&seconds===0){burst();}}
 updateStage();render();
});
